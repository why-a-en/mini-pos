...
// not the stateless HTTP driver (drizzle-orm/neon-http): withVendorScope
 ...
 * policies in docs/DATA_MODEL.md §5 check (`app.vendor_id`) set first. This
 * is defense-in-depth, not the only guard — queries inside `fn` should
 * *also* explicitly filter by vendorId (see docs/DATA_MODEL.md §4 for why
 * it's denormalized onto every table). A bug that skips this call is caught
 * by RLS; a bug that skips the explicit filter is caught by this.
 */
export async function withVendorScope<T>(
  ...
  return db.transaction(async (tx) => {
    await tx.execute(sql`select set_config('app.vendor_id', ${vendorId}, true)`);
